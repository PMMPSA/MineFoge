...
use pomine\command\ConsoleCommandSender;
...
class Main extends PluginBase implements Listener{
	...
	public function onEnable(){
        ...
        if(is_null($this->coin)){
            $this->getLogger()->warning("Can't find plugin CoinAPI");
        }else{
	...
	public function getNamePickaxe($player){
		...
		$this->lv->load($this->getDataFolder() . "user.yml", Config::YAML);
			$pa = "§l§c|§a §aMINE§bFOGE§c ❤§6 ".$this->lv->get($p)["Level"]." §b➡§e ".$p;
		return $pa;
	}
	...
	public function onJoin(PlayerJoinEvent $ev){
		...
		if(!($this->lv->exists($p))){
			...
			$this->lv->set($p, ["Level" => 1, "exp" => 0, "nextexp" => 100]);
			$this->lv->save();
			$this->level->set($p, 1);
	    	$this->level->save();
			$p1 = $ev->getPlayer();
			...
			$inv->addItem($item);
			$player->sendMessage("§l§c•§b Cúp đã được thêm vào túi đồ của bạn, hãy cùng đồng hành với nó lâu nhé, cúp có thể trở thành một cây cúp §eVIP");
			return true;
		}
	}
    ...
    public function onItemHeld(PlayerItemHeldEvent $ev){
       ...
       /* if ($damage > 30) {
        ...
        if(isset($this->need[$p->getName()])){
			$icn = $i->getCustomName();
			$i->setCustomName(str_replace("❤§6 ".($this->lv->get($p->getName())["Level"] - 1), "❤§6 ".$this->lv->get($p->getName())["Level"], $icn));
	...
	$p->setDisplayName("§b[§a".$this->getLevel($p)."§b]§r ".$p->getName());
     ...
     public function onCommand(CommandSender $sender, Command $cmd, $label, array $args){
         if($cmd->getName() == "givecup"){
           if($sender->isOp()){
                if(!isset($args[0])){
                    $sender->sendMessage("§l§b•§a /givecup <Tên Người Chơi>");
                    return true;
                }else{
                    ...
                    if(!$player == null){
                        if($player->isOnline()) {
                            ...
                            $inv->addItem($cup);
                            $player->sendMessage("§l§6⚒§e Cúp MINEFOGE");
                        }
                    }
                }

             }else{
                 $sender->sendMessage("§b§l•§c You don't have permission use this command");
                 return true;
             }
		 ...
		 */
		 $max = 0;
            ...
            $max = count($c);
				...
				$message = "§l§c⚒ §l§bHạng §e".$i1."§b thuộc về §c".$b."§b Với §e".$a." §aCấp §l§c⚒\n\n";
