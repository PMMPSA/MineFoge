...
class Main extends PluginBase implements Listener{
	public function onEnable(){
		...
		$this->coins = new Config($this->getDataFolder()."diamond.yml",Config::YAML);
	...
	public function onCommand(CommandSender $sender, Command $cmd, $label, array $args){
		switch($cmd->getName()){
			case "diamond":
			$sender->sendMessage("§e❖§9 Số Kim Cương:§a ".$this->coins->get($sender->getName()));
			break;		
			case "hmm1":
			if(!isset($args[0])){
					...
					if(!isset($args[0])){
						...
						if(!$player){
							...
							if(!is_numeric($args[1])){
								...
								break;
				case "addkc":
				if($sender->isOp() || $sender->hasPermission("addkc.command")){
					if(!isset($args[0])){
						$sender->sendMessage("§e⇀§6 Nhập lệnh§f /addkc <player> <amount>.");
						return true;
							}			
							$player = $this->getServer()->getPlayer($args[0]);
							if(!$player){
								...
								if(!is_numeric($args[1])){
									...
									$this->coins->save();
									$sender->sendMessage("§a✔§b Đã thêm thành công§6 ".$args[1]."§b Kim Cương vào tài khoản của§e ".$player->getName().".");
									$player->sendMessage("§c❖§b Số Kim Cương của bạn đã được thêm§6 ".$args[1]."§b Kim Cương BỞI ADMIN");
					} else{
						$sender->sendMessage("§cKhông có Quyền!");
						break;
						}
	...
	public function addKc($player, $coins){
   ...
   public function reduceKc($player, $coins){
      if($player instanceof Player){
          $player = $player->getName();
    }
      if($this->myKc($player) - $coins < 0){
          return true;
          }
          $this->coins = new Config($this->getDataFolder()."diamond.yml",Config::YAML);
          $this->coins->set($player, (int)$this->coins->get($player) - $coins);
	...
	public function myKc($player){
		if($player instanceof Player){
			$player = $player->getName();
			}
   $kcapi = $this->coins->get($player);
return $kcapi;
				}
	}