...
class Crate {
     ...
     * @param array $content
     */
    public function __construct(Loader $loader, $name, $identifier, array $content) {
        $this->loader = $loader;
        $this->name = $name;
    ...
    public function execute(CrateBlock $block, Player $player) {
        ...
        if(!$session->isInCrate()) {
            if($session->hasCrateKey($this->identifier)) {
                ...
                $player->sendMessage(TextFormat::DARK_PURPLE . TextFormat::BOLD . "[Crates]" . TextFormat::RESET . " Bạn cần {$this->name}" . TextFormat::RESET . " chìa khóa để mở rương này");
            }
        }
    }
